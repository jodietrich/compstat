...
set.seed(3)
(true.par <- mean(rgamma(100000000, shape = ???, rate = ???), trim = 0.1))
...
###################################################
require("boot")
tm <- function(x, ind) {mean(???, trim = 0.1)}
tm_var <- function(x, ind) {
  # trimmed mean
  t1 <- tm(x, ind) 
  # bootstrap variance of the trimmed mean (required for the bootstrap T CI)
  t2 <- var(boot(data = ???, statistic = ???, R = 50)$t) 
  return(c(t1, t2))
}
res.boot <- boot(data = sample40, statistic = ???, R = 10000, 
                 sim = "ordinary")
boot.ci(res.boot, conf = ???, type = c("basic", "norm", "perc", "stud"), 
...
do_sim <- function(n, true.par, R = 500, 
  ...
  res.boot <- boot(data = x, statistic = ???, R = R, sim = "ordinary")
...
plot(??? ~ ???, data = ???, col = 1, pch = 1, ylim = c(0, y.lim), 
     log = "x", ylab = "One-sided non-coverage", 
     main = "Non-coverage of the upper end of the CIs.") 
points(norm_lower ~ ???, data = ???, col = 2, pch = 2, xlog = TRUE)
points(perc_lower ~ ???, data = ???, col = 3, pch = 3, xlog = TRUE)
points(stud_lower ~ ???, data = ???, col = 4, pch = 4, xlog = TRUE)
lines(basic_lower ~ ???, data = ???, col = ???, lty = ???, xlog = TRUE)
lines(norm_lower ~ ???, data = ???, col = ???, lty = ???, xlog = TRUE)
lines(perc_lower ~ ???, data = ???, col = ???, lty = ???, xlog = TRUE)
lines(stud_lower ~ ???, data = ???, col = ???, lty = ???, xlog = TRUE)
